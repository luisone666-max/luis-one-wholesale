...
PREVIEW_DIR = ROOT / "tmp-imports" / "csl-preview"
PRODUCTS_CSV = PREVIEW_DIR / "csl-products-preview.csv"
VARIANTS_CSV = PREVIEW_DIR / "csl-variants-preview.csv"
PROGRESS_PATH = PREVIEW_DIR / "csl-upload-progress.json"
...
def optimize_image(local_path):
    ...
    try:
        ...
        content_type = mimetypes.guess_type(local_path.name)[0] or ("image/png" if ext == ".png" else "image/jpeg")
...
def storage_upload(local_public_path, image_cache):
    ...
    object_path = f"products/csl/{digest}{ext}"
...
def save_progress(progress):
    ...
    fd, temp_name = tempfile.mkstemp(prefix="csl-upload-progress-", suffix=".json", dir=str(PREVIEW_DIR))
...
def get_or_create_category(name, parent_id=None, level=1, sort_order=0):
    ...
    if existing:
        category = next((item for item in existing if item.get("parent_id") == parent_id), existing[0])
        if category.get("parent_id") != parent_id or category.get("level") != level:
            updated = request_json(
                "PATCH",
                "categories",
                "?id=eq." + urllib.parse.quote(category["id"]) + "&select=*",
                payload={
                    "parent_id": parent_id,
                    "level": level,
                    "active": True,
...
def ensure_category_path(path_text):
    ...
    for index, name in enumerate(names):
        category = get_or_create_category(name, parent["id"] if parent else None, index + 1, 230 + index)
...
def product_sku(row):
    supplier = slugify(row.get("supplier") or "csl").upper()[:16]
    base = slugify(row["english_product_name"]).upper()[:60]
    return f"{supplier}-{base}-{stable_suffix(row['preview_id'], row['english_product_name']).upper()}"[:96]
...
def brand_model_from_name(row):
    name = clean(row["english_product_name"])
    supplier = clean(row.get("supplier")) or "CSL"
    model = ""
    if " - " in name:
        model = clean(name.rsplit(" - ", 1)[1])
    return supplier, model or clean(row.get("source_title"))
...
def product_payload(row, sku, categories, image_url):
    ...
    brand, model = brand_model_from_name(row)
    description = (
        f"{name}. Ready-stock motorcycle part from {brand}. Select the exact model, color, size, or specification variant before ordering. "
    ...
    return {
        ...
        "supplier_notes": None,
        "internal_cost_notes": (
            f"CSL PDF import. PDF NET PRICE is supplier cost. Cost range PHP {cost_min:.2f}-{cost_max:.2f}. "
            f"Retail 1-5pcs starts PHP {retail_min}; 6pcs+ starts PHP {wholesale_min}."
        ),
        "admin_notes": (
            f"Imported from CSL PDF preview: {row['supplier']} / {row['preview_id']}. "
            f"Source: {row.get('source_files_pages')}. Source title: {row.get('source_title')}. "
            f"Review note: {row.get('review_note') or 'none'}."
        ),
        "active": True,
    }
    ...
    for start in range(0, len(rows), batch_size):
        ...
        for offset, row in enumerate(chunk, start=start + 1):
            ...
            fits_parts = [
                ...
                f"Size/spec: {clean(row.get('size'))}" if clean(row.get("size")) else "",
                ...
                f"Source: {clean(row.get('source_file'))} p.{clean(row.get('source_page'))}",
            ]
            payload.append(
                {
...
def fetch_csl_products_page(offset, page_size=1000):
    return request_json(
        "GET",
        "products",
        f"?select=id,sku,admin_notes&admin_notes=ilike.*CSL%20PDF%20preview*&limit={page_size}&offset={offset}",
        prefer=None,
    )


def deactivate_stale_csl_products(current_skus):
    existing = []
    offset = 0
    page_size = 1000
    while True:
        batch = fetch_csl_products_page(offset, page_size)
        if not batch:
            break
        ...
        if len(batch) < page_size:
            break
        offset += page_size
    stale_ids = [item["id"] for item in existing if item.get("sku") not in current_skus]
...
def main():
    parser = argparse.ArgumentParser(description="Slow, resumable import for CSL PDF preview products.")
    parser.add_argument("--limit", type=int, default=0, help="Import only the first N pending products.")
    parser.add_argument("--batch-size", type=int, default=10, help="Pause after this many products.")
    parser.add_argument("--delay", type=float, default=1.0, help="Seconds to pause between product batches.")
    ...
    parser.add_argument("--cleanup-stale", action="store_true", help="Deactivate stale previous CSL PDF products after import.")
    ...
    if args.cleanup_stale and not args.limit:
        summary["stale_csl_products_deactivated"] = deactivate_stale_csl_products(current_skus)
