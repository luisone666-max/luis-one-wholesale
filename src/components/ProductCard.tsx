...
export function ProductCard({ product, priority = false }: { product: Product; priority?: boolean }) {
  ...
  return (
    <article className="group overflow-hidden rounded-sm border border-zinc-200 bg-white shadow-sm transition hover:-translate-y-0.5 hover:border-orange-300 hover:shadow-md">
      <Link href={`/product/${product.slug}`} className="relative block aspect-square bg-gradient-to-br from-orange-50 via-white to-zinc-50 p-0 sm:p-4">
      ...
      <div className="space-y-0.5 p-1.5 sm:space-y-2.5 sm:p-3">
        ...
        <div className="min-h-[34px] sm:min-h-[48px]">
          ...
          <p className="max-w-full truncate text-[11px] font-black leading-4 text-[#f65f18] sm:text-base sm:leading-5" title={getPriceRange(product)}>
